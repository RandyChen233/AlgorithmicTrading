...
while len(tickers) != 0 and attempt <=50000:
    tickers = [j for j in tickers if j not in drop]
    for i in range(len(tickers)):
        try:
            ohlc_intraday[tickers[i]] = ts.get_intraday(symbol=tickers[i],interval='15min', outputsize='full')[0]
...
for ticker in tickers:
    ...
    df[ticker]['data'] = df[ticker].index
    ohlc_renko[ticker] = df[ticker].merge(renko.loc[:,['date','bar_num']],\
                                          how='outer',on='date')
